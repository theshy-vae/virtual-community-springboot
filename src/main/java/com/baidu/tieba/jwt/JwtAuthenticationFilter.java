...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
        ...
        try {
            if(isProtectedUrl(request)) {
                if(!request.getMethod().equals("OPTIONS"))
                    request = JwtUtil.
                            validateTokenAndAddUserIdToHeader(request);
            }
        } catch (Exception e) {
    ...
    private boolean isProtectedUrl(HttpServletRequest request) {
        ...
        protectedPaths.add("/ums/user/updatePass");
        protectedPaths.add("/ums/user/uploadAvatar");
        ...
        protectedPaths.add("/tag/follow");
        protectedPaths.add("/tag/create");
